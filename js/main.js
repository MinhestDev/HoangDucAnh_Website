...
$(window).load(function () {
	...
	if ($('#front-slider').length > 0) {
		$('#front-slider').fractionSlider({
			...
			'startCallback': function(el, currentSlide, lastSlide, step ){
				$('.topslider .slider .prev').text('Lùi').appendTo('.topslider');
				$('.topslider .slider .next').text('Tiến').appendTo('.topslider');
	...
	if ($("#theme-options").length > 0) {
		$("#theme-options").on("click", '#theme-options-toggle', function(e) {
			$("#theme-options").toggleClass("opened");
			return false;
		});
		$('body').on('click', function() {
			if ($('#theme-options').length > 0 && $('#theme-options').hasClass('opened')) {
				$('#theme-options').removeClass('opened');
			}
		});
		$('#theme-options').on('click', function(event){
		    event.stopPropagation();
		});
		$('#theme-options .theme-colors').on('click', 'li', function(e) {
			if ($(this).hasClass('active')) {
				return false;
			} else {
				$('#theme-options .theme-colors li').removeClass('active');
				$(this).addClass('active');
				$('#theme-color').attr('href', 'css/color/' + $(this).data('color') + '.css');
			}
			return false;
		});
	}
